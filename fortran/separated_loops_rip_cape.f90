...
! NCLFORTEND
    ...
    REAL(KIND=8) :: davg, ethmax, q, t, p, e, eth, zlcl, tlcl
    REAL(KIND=8) :: pavg, tvirtual, p1, p2, pp1, pp2, th, totthe, totqvp, totprs
    REAL(KIND=8) :: cpm, deltap, gammam, ghtpari, qvppari, prspari, tmkpari, ethpari
    ...
    REAL(KIND=8), DIMENSION(miy,mjx,mkzh) :: ethpari1
    REAL(KIND=8), DIMENSION(miy,mjx,mkzh) :: tlcl1
    REAL(KIND=8), DIMENSION(miy,mjx,mkzh) :: tmklift_new1
    REAL(KIND=8), DIMENSION(miy,mjx,mkzh) :: zlcl1
...
!!$OMP SIMD          
           DO kpar = 2, mkzh
             ...
             ! gammam = GAMMA * (1.D0 + GAMMAMD*qvp(i,j,kpar))
             ! cpm = CP * (1.D0 + CPMD*qvp(i,j,kpar))

              tlcl1(i,j,kpar) = TLCLC1/(LOG(tmk(i,j,kpar)**TLCLC2/(MAX(1.D-20,qvp(i,j,kpar)*prs(i,j,kpar)/(EPS + qvp(i,j,kpar))))) - TLCLC3) + TLCLC4
             
              ethpari1(i,j,kpar) = tmk(i,j,kpar)*(1000.D0/prs(i,j,kpar))**(GAMMA*(1.D0 + GAMMAMD*qvp(i,j,kpar)))* &
                  EXP((THTECON1/tlcl - THTECON2)*qvp(i,j,kpar)*(1.D0 + THTECON3*qvp(i,j,kpar)))
              
              zlcl1(i,j,kpar) = ght(i,j,kpar) + (tmk(i,j,kpar) - tlcl)/(G/CP * (1.D0 + CPMD*qvp(i,j,kpar)))

               DO k = kpar,1,-1
                  tmklift_new1(i,j,k) = TONPSADIABAT(ethpari1(i,j,kpar), prs(i,j,k), psadithte, psadiprs,&
              ...
              DO k = kpar,1,-1   
               ! For arrays that go bottom to top
                  ...
                  IF (ght(i,j,k) .LT. zlcl) THEN
                      qvplift = qvp(i,j,kpar)
                      tmklift = tmk(i,j,kpar) - G/(CP * (1.D0 + CPMD*qvp(i,j,kpar)))*(ght(i,j,k) - ght(i,j,kpar))
                      tvenv = tmk(i,j,k)*(EPS + qvp(i,j,k))/(EPS*(1.D0 + qvp(i,j,k)))
                      tvlift = tmklift*(EPS + qvp(i,j,kpar))/(EPS*(1.D0 + qvp(i,j,kpar))) 
                      ghtlift = ght(i,j,k)
                  ELSE IF (ght(i,j,k) .GE. zlcl .AND. ilcl .EQ. 0) THEN
                      ...
                      tmklift = tlcl1(i,j,kpar)
                      qvplift = qvp(i,j,kpar)
                      facden = 1/(ght(i,j,k) - ght(i,j,k+1))
                      fac1 = (zlcl1(i,j,kpar)-ght(i,j,k+1))*facden
                      fac2 = (ght(i,j,k)-zlcl1(i,j,kpar))*facden
                      tmkenv = tmk(i,j,k+1)*fac2 + tmk(i,j,k)*fac1
                      qvpenv = qvp(i,j,k+1)*fac2 + qvp(i,j,k)*fac1
                      tvenv = tmkenv* (EPS + qvpenv) / (EPS * (1.D0 + qvpenv))
                      tvlift = tlcl1(i,j,kpar)* (EPS + qvp(i,j,kpar)) / (EPS *(1.D0 + qvp(i,j,kpar)))
                      ghtlift = zlcl1(i,j,kpar)
                      ilcl = 1
                  ELSE
                      tmklift = tmklift_new1(i,j,k)
