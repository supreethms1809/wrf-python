...
DO i = 1,mjx
   DO j = 1,miy
      DO k = 1,mkzh 
         prs_new(k,j,i) = prs(i,j,k)
         tmk_new(k,j,i) = tmk(i,j,k)
         qvp_new(k,j,i) = qvp(i,j,k)
         ght_new(k,j,i) = ght(i,j,k)
      END DO
   END DO
END DO
...
!$OMP SIMD          
           DO kpar = 2, mkzh
              ...
              ! (note, qvppari and tmkpari already calculated above for 2d case.)
                  ...
                  qvppari = qvp_new(kpar,i,j)
                  tmkpari = tmk_new(kpar,i,j)
              prspari = prs_new(kpar,i,j)
              ghtpari = ght_new(kpar,i,j)
             ! gammam = GAMMA * (1.D0 + GAMMAMD*qvppari)
              gammam = GAMMA * (1.D0 + GAMMAMD*qvp_new(kpar,i,j))
             ! cpm = CP * (1.D0 + CPMD*qvppari)
              cpm = CP * (1.D0 + CPMD*qvp_new(kpar,i,j))
